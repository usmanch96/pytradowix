...
class HistoryMixin(_ClientBase):
    ...
    async def get_historical_candles(
        ...
        async with lock:
            ...
            while current_end_time > target_start_time and chunk_index < MAX_CHUNKS:
                chunk_index += 1
                logger.info(
                    f"Fetching chunk {chunk_index} for {symbol} "
                    f"ending at {current_end_time:.0f} (Target start: {target_start_time:.0f})"
                )
                try:
